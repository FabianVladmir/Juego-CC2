...
void init() {
	...
	tutorialText.setString("abajo para disparar y comenzar la partida, arriba saltar");
...
void update(float dt) {
	...
	for (int i = 0; i < monkeys.size(); i++) {
		...
		if (monkey->getSprite().getPosition().x < 0)
		{
	...
	// Check collision between Rocket and Enemies or Rocket and Monkey
	...
	for (int i = 0; i < rockets.size(); i++) {
		for (int j = 0; j < monkeys.size(); j++) {
			...
			if (checkCollision(rocket->getSprite(), monkey->getSprite())) {
				...
				rockets.erase(rockets.begin() + i);
				monkeys.erase(monkeys.begin() + j);

				delete(rocket);
				delete(monkey);

				printf(" rocket intersects monkey \n");
			}
...
void spawnMonkey() {
	...
	sf::Vector2f monkeyPos;

	float speed(0.0);

	switch (randLoc) {
	
	case 0: monkeyPos = sf::Vector2f(viewSize.x, viewSize.y * 0.80f); speed = -700; break;
	case 1: monkeyPos = sf::Vector2f(viewSize.x, viewSize.y * 0.65f); speed = -700; break;
	default: printf("incorrect y value \n"); break;
	}

	Monkey* monkey = new Monkey();
	monkey->init("Assets/graphics/monkeys.png", monkeyPos, speed);
